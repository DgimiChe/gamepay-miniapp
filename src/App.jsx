...
// Иконки для категорий продуктов (масштабируемый маппинг)
...
// Иконка по умолчанию для продукта
function ProductIcon({ category, size = 38 }) {
  const iconType = getProductIconType(category);
  const IconComponent = CATEGORY_ICONS[iconType] || IcoGiftCard;
  return <IconComponent size={size} />;
}

// ─── МОК-ЗАКАЗЫ (fallback при отсутствии API) ───────────────────
const mockOrders = [
...
// ─── КАРТОЧКА ТОВАРА (адаптирована под API) ────────────────────
function ProductCard({ product, onClick }) {
  const [hov, setHov] = useState(false);

  const name = product.name || "Товар";
  const region = product.category || "🌍 Глобальный";
  const minPrice = product.skus?.length 
    ? Math.min(...product.skus.map(s => s.price_rub || 0))
    : 0;
  const soldCount = product.sold_count || Math.floor(Math.random() * 500 + 50);
  ...
  return (
    <div onClick={() => onClick(product)}
      ...
      <div style={{ marginBottom: 10 }}><ProductIcon category={product.category} size={38} /></div>
      <div style={{ fontSize: 13, fontWeight: 700, color: C.text, marginBottom: 4, lineHeight: 1.3 }}>{name}</div>
      <div style={{ fontSize: 11, color: C.muted, marginBottom: 10 }}>{region}</div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <span style={{ fontSize: 14, fontWeight: 700, color: C.accentLight }}>от {minPrice} ₽</span>
        <span style={{ fontSize: 10, color: C.muted }}>×{soldCount}</span>
      </div>
    </div>
  );
}
...
function OnboardingScreen({ onDone }) {
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", padding: "32px 24px 24px", minHeight: 0 }}>
      <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", textAlign: "center", minHeight: 0 }}>
        <div style={{ marginBottom: 24 }}><s.Icon size={72} /></div>
        <div style={{ fontSize: 20, fontWeight: 800, color: C.text, marginBottom: 10, lineHeight: 1.3 }}>{s.title}</div>
        <div style={{ fontSize: 13, color: C.muted, lineHeight: 1.7, maxWidth: 280 }}>{s.text}</div>
      </div>
      <div style={{ display: "flex", gap: 8, justifyContent: "center", marginBottom: 20 }}>
        {slides.map((_, i) => (
          <div key={i} onClick={() => setSlide(i)}
            style={{ width: i === slide ? 24 : 8, height: 8, borderRadius: 4, background: i === slide ? C.accent : C.border, transition: "all 0.3s", cursor: "pointer", boxShadow: i === slide ? `0 0 8px ${C.accent}` : "none" }} />
        ))}
      </div>
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        <button onClick={() => slide < slides.length - 1 ? setSlide(slide + 1) : onDone()}
          ...
          {slide < slides.length - 1 ? "Далее →" : "Начать покупки ⚡"}
        </button>
...
// ─── ЭКРАН: ГЛАВНАЯ (HomeScreen) — с API-данными ───────────────
function HomeScreen({ onProduct }) {
  const [cat, setCat] = useState("all");
  const [search, setSearch] = useState("");

  // Получаем категории из конфига (масштабируемо)
  const cats = Object.entries(CATEGORY_MAP).map(([id, config]) => ({
    id,
    label: config.label,
  ...
  // Иконка категории
  const getCatIcon = (iconType) => {
    if (iconType === "games") return <IcoGames size={16} />;
    if (iconType === "gift") return <IcoGiftCard size={16} />;
    if (iconType === "subs") return <IcoSubs size={16} />;
    return null;
  };

  // Получаем данные из контекста App
  const { catalog, catalogLoading, catalogError, loadCatalog } = React.useContext(AppContext);

  // Фильтрация по категории и поиску
  const filtered = (catalog || []).filter(p => {
    const matchesCategory = cat === "all" || getUiCategory(p.category) === cat;
    const matchesSearch = p.name?.toLowerCase().includes(search.toLowerCase());
    return matchesCategory && matchesSearch;
  });

  // Показываем skeleton пока грузится
  if (catalogLoading) return <SkeletonScreen />;

  // Показываем ошибку с возможностью retry
...
// ─── ЭКРАН: ТОВАР (ProductScreen) — с API-данными ──────────────
function ProductScreen({ product, onBack, onBuy }) {
  const [sel, setSel] = useState(0);
  const [pid, setPid] = useState("");

  const variants = product.skus || [];
  const needsId = product.attributes?.requires_id || product.category?.toLowerCase().includes("game") || false;
  const description = product.description || "Цифровой товар с мгновенной выдачей.";
  const region = product.category || "🌍 Глобальный";
  const regionNote = product.region_note || `Для ${region} региона`;
  const deliveryTime = product.delivery_time || "Мгновенно";
  const validity = product.validity || "Бессрочно";
  const compatibility = product.compatibility || "Любой аккаунт";
  const instruction = product.instruction || ["Открой приложение", "Найди раздел «Погасить код»", "Введи полученный код"];
  const guarantee = product.guarantee !== false;
  const badge = product.badge || null;

  // Цвет по категории
  const getColor = () => {
    const cat = getUiCategory(product.category);
    if (cat === "games") return "#F59E0B";
    if (cat === "gift") return "#22C55E";
    if (cat === "subs") return "#3B82F6";
    return "#7C3AED";
  };
  ...
  return (
    <div style={{ flex: 1, overflowY: "auto", padding: "0 16px 24px" }}>
      ...
      {/* Шапка карточки */}
      <div style={{ background: `linear-gradient(135deg, ${color}22, ${C.card})`, border: `1px solid ${color}44`, borderRadius: 18, padding: 20, marginBottom: 16, textAlign: "center" }}>
        <div style={{ display: "flex", justifyContent: "center", marginBottom: 12 }}><ProductIcon category={product.category} size={56} /></div>
        {badge && (
          <div style={{ marginBottom: 8 }}>
            <span style={{ background: C.warning + "22", color: C.warning, fontSize: 11, fontWeight: 700, padding: "3px 10px", borderRadius: 20, border: `1px solid ${C.warning}44` }}>{badge}</span>
          </div>
        )}
        <div style={{ fontSize: 20, fontWeight: 800, color: C.text, marginBottom: 8 }}>{product.name}</div>
        <div style={{ display: "flex", gap: 8, justifyContent: "center", flexWrap: "wrap" }}>
          <Badge color={color}>{region}</Badge>
          {guarantee && <Badge color={C.success}>Гарантия замены</Badge>}
        </div>
      </div>

      {/* Описание */}
      <div style={{ fontSize: 13, color: C.muted, lineHeight: 1.7, marginBottom: 16 }}>{description}</div>

      {/* Характеристики */}
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 16 }}>
        {[
          { label: "Выдача", value: deliveryTime },
          { label: "Срок", value: validity },
          { label: "Аккаунт", value: compatibility.length > 18 ? compatibility.slice(0, 16) + "…" : compatibility },
        ].map(({ label, value }) => (
      ...
      <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: "12px 14px", marginBottom: 16, display: "flex", gap: 10, alignItems: "flex-start" }}>
        <IcoSupport size={20} />
        <div style={{ fontSize: 13, color: C.text }}>{regionNote}</div>
      </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {instruction.map((step, i) => (
            <div key={i} style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
              <div style={{ minWidth: 22, height: 22, borderRadius: 11, background: C.accent + "33", border: `1px solid ${C.accent}55`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 700, color: C.accentLight }}>{i + 1}</div>
      ...
      <div style={{ display: "flex", justifyContent: "center", gap: 16, marginTop: 14 }}>
        {["СБП", "Карта РФ", "ЮMoney"].map(m => <span key={m} style={{ fontSize: 11, color: C.muted }}>{m}</span>)}
      </div>
    </div>
  );
}
...
function PaymentScreen({ product, variantIdx, onSuccess, onBack }) {
  ...
  const variant = product.skus?.[variantIdx];
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      if (err instanceof NetworkError) message = "Нет соединения. Проверь интернет.";
      else if (err instanceof TimeoutError) message = "Сервер долго отвечает. Попробуй позже.";
      else if (err instanceof ValidationError) message = err.message;
  ...
  if (error) return (
    <div style={{ flex: 1, overflowY: "auto", padding: "0 16px 24px" }}>
      ...
      <ErrorScreen message={error} onRetry={() => setError(null)} onHome={onBack} />
    </div>
  );
  ...
  if (step === 1) return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 32 }}>
      ...
      <div style={{ fontSize: 17, fontWeight: 700, color: C.text, marginBottom: 8 }}>Создаём заказ...</div>
      <div style={{ width: "100%", height: 3, background: C.border, borderRadius: 4, overflow: "hidden", marginTop: 20 }}>
        <div style={{ height: "100%", background: `linear-gradient(90deg, ${C.accent}, ${C.accentLight})`, borderRadius: 4, boxShadow: `0 0 10px ${C.accent}`, animation: "prg 2s linear forwards" }} />
      </div>
  ...
  if (step === 2) return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 32 }}>
      <IcoSuccess size={64} />
      <div style={{ fontSize: 16, color: C.success, marginTop: 12 }}>Заказ создан!</div>
      <div style={{ fontSize: 12, color: C.muted, marginTop: 8 }}>Перенаправляем на оплату...</div>
    </div>
  );
      ...
      <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, padding: 16, marginBottom: 20 }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            ...
            <div style={{ fontSize: 13, color: C.muted }}>{variant?.name} · {product.category}</div>
          </div>
          <div style={{ fontSize: 22, fontWeight: 800, color: C.accentLight }}>{variant?.price_rub} ₽</div>
        </div>
      </div>
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "24px 20px", textAlign: "center" }}>
      ...
      <div style={{ fontSize: 14, color: C.muted, marginBottom: 28 }}>
        {needsId ? "Пополнение зачислено на аккаунт" : "Твой код активации:"}
      </div>
...
function ProfileScreen({ orders, onSupport }) {
  ...
  const refLink = `https://t.me/gameandpay_bot?start=ref_${userId}`;
    ...
    <div style={{ flex: 1, overflowY: "auto", padding: "16px" }}>
      ...
      <div style={{ background: `linear-gradient(135deg, ${C.accent}33, ${C.card})`, border: `1px solid ${C.accent}44`, borderRadius: 16, padding: 20, marginBottom: 16, display: "flex", alignItems: "center", gap: 14, boxShadow: `0 0 24px ${C.accent}22` }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 12, color: C.muted }}>ID: {userId}</div>
        </div>
      </div>
      ...
      <div style={{ background: C.card, border: `1px solid ${C.warning}44`, borderRadius: 14, padding: 16, marginBottom: 12 }}>
        ...
        <div style={{ background: C.bg, borderRadius: 8, padding: "8px 12px", fontFamily: "monospace", fontSize: 11, color: C.text, marginBottom: 10, border: `1px solid ${C.border}`, wordBreak: "break-all" }}>{refLink}</div>
        <button onClick={copyRefLink}
          style={{ width: "100%", background: refCopied ? C.success + "22" : C.warning + "22", border: `1px solid ${refCopied ? C.success : C.warning}44`, borderRadius: 10, padding: "10px", color: refCopied ? C.success : C.warning, fontSize: 13, fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, transition: "all 0.3s" }}>
          <IcoCopy size={18} /> {refCopied ? "Скопировано!" : "Скопировать ссылку"}
        </button>
      </div>
      ...
      <div style={{ background: C.card, border: `1px solid ${C.success}44`, borderRadius: 14, padding: 16, marginBottom: 12 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
          <IcoCashback size={20} />
          <div style={{ fontSize: 13, fontWeight: 700, color: C.success }}>Кешбэк: {cashback} ₽</div>
        </div>
      ...
      {status === "sent" ? (
        ...
        <>
          ...
          <textarea
            ...
            rows={6}
            style={{ width: "100%", background: C.card, border: `1px solid ${text ? C.accent + "66" : C.border}`, borderRadius: 12, padding: "12px 14px", color: C.text, fontSize: 13, outline: "none", resize: "none", boxSizing: "border-box", fontFamily: "'Inter', -apple-system, sans-serif", lineHeight: 1.6, transition: "border-color 0.2s", marginBottom: 12 }}
          />
          ...
          <button onClick={sendMessage} disabled={!text.trim() || status === "sending"}
            style={{ width: "100%", border: "none", borderRadius: 14, padding: "15px", background: !text.trim() ? C.border : `linear-gradient(135deg, ${C.accent}, #9333EA)`, color: "#fff", fontSize: 15, fontWeight: 700, cursor: !text.trim() ? "not-allowed" : "pointer", boxShadow: text.trim() ? `0 4px 20px ${C.accent}55` : "none", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
...
// ─── ЭКРАН: ДЕТАЛИ ЗАКАЗА (OrderDetailScreen) ──────────────────
...
// ─── REACT CONTEXT для передачи данных между экранами ───────────
...
export default function App() {
  ...
  // Заказы из localStorage (fallback) или API (будущее)
  ...
  const loadCatalog = useCallback(async () => {
    ...
    try {
      ...
      setRate(data.rate || 95);
      setMarkupPercent(data.markup_percent || 15);
    } catch (err) {
      if (cancelled) return;

      let message = "Не удалось загрузить каталог";
      if (err instanceof NetworkError) {
        message = "Нет соединения. Проверь интернет и попробуй снова.";
      } else if (err instanceof TimeoutError) {
        message = "Сервер долго отвечает. Попробуй позже.";
      } else if (err instanceof ValidationError) {
        message = "Ошибка в данных каталога.";
  ...
  // ─── РЕНДЕР ЭКРАНА ──────────────────────────────────────────
  const Screen = () => {
    if (catalogLoading && screen === "home") return <SkeletonScreen />;
    if (catalogError && screen === "home") return <ErrorScreen message={catalogError} onRetry={loadCatalog} onHome={() => {}} />;
    if (error) return <ErrorScreen message={error} onRetry={() => setError(null)} onHome={goHome} />;
  ...
  if (!onboarded) {
    return (
      <div style={{ display: "flex", justifyContent: "center", alignItems: isTelegram ? "flex-start" : "center", minHeight: "100vh", background: "#050510", fontFamily: "'Inter', -apple-system, sans-serif" }}>
        <div style={{ width: isTelegram ? "100%" : 375, height: isTelegram ? "100vh" : 720, background: C.bg, borderRadius: isTelegram ? 0 : 36, display: "flex", flexDirection: "column", boxShadow: isTelegram ? "none" : "0 30px 80px rgba(0,0,0,0.8), 0 0 0 1px rgba(255,255,255,0.05)" }}>
  ...
  const contextValue = {
    ...
    rate, markupPercent, orders, setOrders
  };

  return (
    <AppContext.Provider value={contextValue}>
      <div style={{ display: "flex", justifyContent: "center", alignItems: isTelegram ? "flex-start" : "center", minHeight: "100vh", background: "#050510", fontFamily: "'Inter', -apple-system, sans-serif", overflowY: isTelegram ? "auto" : "hidden" }}>
        <div style={{ width: isTelegram ? "100%" : 375, height: isTelegram ? "100vh" : 720, maxWidth: isTelegram ? "none" : 375, background: C.bg, borderRadius: isTelegram ? 0 : 36, overflow: "hidden", display: "flex", flexDirection: "column", boxShadow: isTelegram ? "none" : "0 30px 80px rgba(0,0,0,0.8), 0 0 0 1px rgba(255,255,255,0.05)" }}>
