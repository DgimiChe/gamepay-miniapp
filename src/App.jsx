...
function ProfileScreen({ orders }) {
  ...
  const openSupport = () => {
    // Закрываем Mini App — Telegram вернёт пользователя в чат с ботом
    const tg = window.Telegram?.WebApp;
    if (tg) {
      tg.close();
    } else {
      // Фоллбэк для веб-превью
  ...
  return (
    <div style={{ flex: 1, overflowY: "auto", padding: "16px" }}>
      ...
      <div style={{ background: C.card, border: `1px solid ${C.warning}44`, borderRadius: 14, padding: 16, marginBottom: 12 }}>
        ...
        <div style={{ background: C.bg, borderRadius: 8, padding: "8px 12px", fontFamily: "monospace", fontSize: 11, color: C.text, marginBottom: 10, border: `1px solid ${C.border}`, wordBreak: "break-all" }}>
          {refLink}
        </div>
...
function OnboardingScreen({ onDone }) {
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", padding: "32px 24px 24px", minHeight: 0 }}>
      {/* Слайд — занимает доступное место */}
...
export default function App() {
  ...
  if (!onboarded) {
    ...
    return (
      <div style={{
        display: "flex", justifyContent: "center", alignItems: isTg ? "flex-start" : "center",
        minHeight: "100vh", background: "#050510",
        fontFamily: "'Inter', -apple-system, sans-serif",
      }}>
        <div style={{
          width: isTg ? "100%" : 375,
          height: isTg ? "100vh" : 720,
          background: C.bg,
          borderRadius: isTg ? 0 : 36,
  ...
  const Screen = () => {
    ...
    if (error) return <ErrorScreen message={error} onRetry={() => { setLoading(true); setTimeout(() => { setLoading(false); setError(null); }, 800); }} onHome={goHome} />;
  ...
  return (
    <div style={{
      display: "flex", justifyContent: "center", alignItems: isTelegram ? "flex-start" : "center",
      minHeight: "100vh", background: "#050510",
      fontFamily: "'Inter', -apple-system, sans-serif",
      overflowY: isTelegram ? "auto" : "hidden",
    }}>
      <div style={{
        width: isTelegram ? "100%" : 375,
        height: isTelegram ? "100vh" : 720,
        maxWidth: isTelegram ? "none" : 375,
        background: C.bg,
        borderRadius: isTelegram ? 0 : 36,
        overflow: "hidden",
        display: "flex",
        flexDirection: "column",
        boxShadow: isTelegram ? "none" : "0 30px 80px rgba(0,0,0,0.8), 0 0 0 1px rgba(255,255,255,0.05)",
      }}>
