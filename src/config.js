...
// apiCategories — точные строки из API или подстроки для поиска
...
export function getUiCategory(apiCategory) {
  ...
  for (const [uiId, config] of Object.entries(CATEGORY_MAP)) {
    ...
    // Проверяем точное совпадение или подстроку
    for (const cat of config.apiCategories) {
      if (lower === cat.toLowerCase()) return uiId;
      // Проверяем подстроку (например "Пополнение" найдётся в "Пополнение мобильной связи")
      if (lower.includes(cat.toLowerCase()) || cat.toLowerCase().includes(lower)) return uiId;
    }
  }
  return null; // неизвестная категория — показывается только во "Все"
}
